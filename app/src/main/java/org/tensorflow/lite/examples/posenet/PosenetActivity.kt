...
class PosenetActivity :
  ...
  private lateinit var jsonPoints:JsonPoints
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    val conf =
      (context!!.getSharedPreferences("SharedPrefs", Context.MODE_PRIVATE)?.getFloat("minConfidence", 0.5f) as Float)
    Log.d(TAG, "onViewCreated: Confidence${conf}")
    minConfidence = conf.toDouble()
  ...
  override fun onDestroy() {
    val pst: POST = POST((context!!.getSharedPreferences("SharedPrefs", Context.MODE_PRIVATE)?.getString("TOKEN", "") as String))
  ...
  private fun draw(canvas: Canvas, person: Person, bitmap: Bitmap) {
    ...
    for (keyPoint in person.keyPoints) {
      if (keyPoint.score > minConfidence) {
        jsonPoints.add(keyPoint,frameCounter++)
